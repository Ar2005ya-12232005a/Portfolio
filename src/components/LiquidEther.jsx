...
/**
 * LiquidEther — Theme-aware fluid background
 *
 * Props:
 *   theme="dark"   → black background + grey metallic fluid
 *   theme="light"  → white background + soft white/grey fluid
 *
 * Usage:
 *   <LiquidEther theme={isDark ? 'dark' : 'light'} />
 */
export default function LiquidEther({
  ...
  // derive palette & bg from theme
  const isDark = theme !== 'light';
  const colors = isDark
  ? ['#1a1a1a', '#e1e1e1', '#c8c7c8', '#e4e3e5', '#e1e1e1']  // dark: near-black → silver
    : ['#ffffff', '#f5f5f5', '#e0e0e0', '#cccccc', '#f8f8f8'];  // light: white → soft grey

  const bgRGBA = isDark
    ? new THREE.Vector4(0, 0, 0, 1)       // solid black
    : new THREE.Vector4(1, 1, 1, 1);      // solid white

  useEffect(() => {
    if (!mountRef.current) return;

    function makePaletteTexture(stops) {
      const arr = Array.isArray(stops) && stops.length
    ...
    class AutoDriver {
      constructor(mouse,manager,opts){
        ...
        this.margin=0.2;this._tmpDir=new THREE.Vector2();
        this.pickNewTarget();
      }
    ...
    class ExternalForce extends ShaderPass {
      ...
      update(props){
        const fx=(Mouse.diff.x/2)*props.mouse_force,fy=(Mouse.diff.y/2)*props.mouse_force;
        ...
        this.output=new THREE.Mesh(
          ...
          new THREE.RawShaderMaterial({
            ...
            uniforms:{
              ...
              bgColor:{value:bgRGBA}   // ← theme-aware bg injected here
            }
          })
        );
  ...
  return (
    <div
      ref={mountRef}
      className={`w-full h-full relative overflow-hidden pointer-events-none touch-none${className?' '+className:''}`}
      style={style}
    />
  );
}